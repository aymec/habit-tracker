...
export default function TabLayout() {
  ...
  const effectiveWidth = Platform.OS === 'web' && !mounted ? 0 : windowWidth;
  const effectiveFontScale = Platform.OS === 'web' && !mounted ? 1 : fontScale;
  const effectiveInsetsBottom = Platform.OS === 'web' && !mounted ? 0 : insets.bottom;

  // On web, when window is wide enough, labels appear next to icons - use larger font
  const TAB_LABEL_SIDE_BY_SIDE_THRESHOLD = 768;
  const isWebWideLabelLayout = Platform.OS === 'web' && effectiveWidth >= TAB_LABEL_SIDE_BY_SIDE_THRESHOLD;
    ...
    <Tabs
      screenOptions={{
        ...
        tabBarStyle: {
          ...
          minHeight: (Platform.OS === 'ios' ? 40 : 50) + 15 * effectiveFontScale + effectiveInsetsBottom, // Scale gently with font size for accessibility
          paddingBottom: effectiveInsetsBottom > 0 ? 20 + effectiveInsetsBottom : 10, // Push content above nav bar
          paddingTop: 10,
          ...(Platform.OS === 'web' && {
